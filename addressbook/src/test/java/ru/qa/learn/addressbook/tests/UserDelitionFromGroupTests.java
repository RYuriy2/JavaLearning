...
public class UserDelitionFromGroupTests extends TestBase {
    ...
    public void ensurePrecinditions() {
        ...
        Users users2 = new Users();                                             //Получаем список всех существующих пользователей
        ...
        for (UserData user : users) {                                //То по всему списку пользователей
            if (user.getGroups().size() == 0) {                      //Проверяем, состоит ли он хотя бы в одной группе
                users2.add(user);                                 //Удаляем из списка пользователей тех, кто не состоит ни в одной группе
            }
        }
    ...
    public void userDelitionFromGroupTest() {
        ...
        Users users = app.db().users();
        Users usersWithoutGroup = new Users();                                   //Выбираем пользователя, с которым будем работать
        for (UserData user : users) {                                            //То по всему списку пользователей
            if (user.getGroups().size() == 0) {                                  //Проверяем, состоит ли он хотя бы в одной группе
                usersWithoutGroup.add(user);                                             //Удаляем из списка пользователей тех, кто не состоит ни в одной группе
            }
        }
        users.removeAll(usersWithoutGroup);
